...
def predict(C, alpha, X, y):
    ...
    for i in range(y.shape[0]):
        pyp = float(C[0][0]) / float(C[2][0])
        pyn = float(C[1][0]) / float(C[2][0])

        for j in range(1, X_n):
            if X[i][j] == 0:
                pyp *= (float(C[0][0]) - float(C[0][j]) + alpha) / (float(C[0][0]) + alpha * 2)
                pyn *= (float(C[1][0]) - float(C[1][j]) + alpha) / (float(C[1][0]) + alpha * 2)
            if X[i][j] == 1:
                pyp *= (float(C[0][j]) + alpha) / (float(C[0][0]) + alpha * 2)
                pyn *= (float(C[1][j]) + alpha) / (float(C[1][0]) + alpha * 2)
